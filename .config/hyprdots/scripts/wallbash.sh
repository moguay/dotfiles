...
# skipping if already exists with good colors settings

if [ -f ${wallbashOut} ]; then
    # read theme wallbash colors
    source "${wallbashOut}"

    # Calculate the right-hand side of the equation
    right_value=$((3 + ${wallbashColors}*2 + ${wallbashColors}*${wallbashAccent}))

    # Get the number of lines in the file
    num_lines=$(wc -l < "${wallbashOut}")

    # Check for skipping
    if [ "${num_lines}" -eq "${right_value}" ] && [ "${bashColors}" = "${wallbashColors}" ] && [ "${bashAccent}" = "${wallbashAccent}" ] && [ "${bashFuzz}" = "${wallbashFuzz}" ]; then
        exit 0
    fi
fi
...
# store wallbash settings

echo "bashColors=${wallbashColors}" >> "${wallbashOut}"
echo "bashAccent=${wallbashAccent}" >> "${wallbashOut}"
echo "bashFuzz=${wallbashFuzz}" >> "${wallbashOut}"
